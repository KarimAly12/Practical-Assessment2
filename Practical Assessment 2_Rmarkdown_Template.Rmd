...
numeric_columns <- merged_df %>% select(price, bed, bath, `house_size`, price_per_sqft)

# Plot boxplots for each column to visually inspect outliers
boxplot_list <- lapply(names(numeric_columns), function(col) {
  ggplot(merged_df, aes_string(x = "1", y = col)) +
    geom_boxplot() +
...
# Function to detect outliers using IQR method (with na.rm = TRUE to handle missing values)
detect_outliers <- function(x) {
  ...
  return(x[x < lower_bound | x > upper_bound & !is.na(x)])
}

# Apply the outlier detection function to each numeric column (after handling NAs)
outliers <- lapply(numeric_columns, detect_outliers)

# Print outliers for each column
names(outliers) <- names(numeric_columns)
outliers
```
...
# Check the skewness of the 'price' variable before transformation
library(ggplot2)

# Plotting the original distribution of 'price'
ggplot(merged_df, aes(x = price)) +
  geom_histogram(bins = 30, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Price (Before Transformation)", x = "Price", y = "Count")
...
ggplot(merged_df, aes(x = log_price)) +
...
# Compare the original and transformed variables side by side
summary(merged_df$price)
summary(merged_df$log_price)
