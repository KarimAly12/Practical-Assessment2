...
# Add an index column for the row number
merged_df$index <- 1:nrow(merged_df)

# Check for outliers using scatter plots
...
# Add a new column in the data to identify outliers for each numeric column
for (col in names(numeric_columns)) {
  merged_df[[paste0(col, "_outlier")]] <- detect_outliers(merged_df[[col]])
}

# Plot scatter plots for each column with outliers highlighted
scatter_plot_list <- lapply(names(numeric_columns), function(col) {
  ggplot(merged_df, aes_string(x = "index", y = col, color = paste0(col, "_outlier"))) +
    geom_point(alpha = 0.6) +
    scale_color_manual(values = c("black", "red")) +  # Red for outliers, black for normal points
    labs(
      title = paste("Scatter Plot of", col, "with Outliers Highlighted"), 
      x = "Index", 
      y = col, 
      color = "Outlier"
    ) +
